...
class TestStringMethods:
    ...
    def test_mapping(self, data_dir, id_to_entity, id_to_relation) -> None:
        ...
        for filename in filenames:
            filename_raw = path.join(data_dir, '../../raw/FB15k-237', filename)

            with open(path.abspath(filename_raw), 'r') as file:
                raw_triples = [l.strip().split('\t') for l in file.readlines()]

            filename_triples = path.join(data_dir, f"{filename[:-4]}.pickle")
