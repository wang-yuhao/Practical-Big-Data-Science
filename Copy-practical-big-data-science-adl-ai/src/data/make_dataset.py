...
def main(input_filepath: str = "./data",
    ...
    if path.isdir(raw_data_dir):
        ...
        logger.info(f'File or directory does not exist: {raw_data_dir}')
...
def create_index(filenames: list, raw_data_dir: str,
    ...
    logger = logging.getLogger(__name__)
    logger.info(f"Found {len(entities)} different entities")
    logger.info(f"Found {len(relations)} different relations")
